...
public class Finance {
     ...
     * 存放在虚拟机的位置
     */
    private final static String path = "/data/server/springboot";
    ...
    static {
        ...
        if (os.indexOf("Win")!=-1) {
            ...
            //linux环境
            loadLib("libs/libWeWorkFinanceSdk_Java.so");
        }
    }
    ...
    public static void loadLib(String libName) {
        String tmpName = libName.replace("libs/","");

        String resourcePath = "/" + libName;
        String folderName = path + File.separator+"libs"+File.separator;
        File folder = new File(folderName);
        if (!folder.exists()){
            folder.mkdirs();
        }

        File libFile = new File(folder, tmpName);

        logger.info("文件位置："+libFile);
        if (libFile.exists()) {
            System.load(libFile.getAbsolutePath());
        } else {
            try {

                InputStream in = Finance.class.getResourceAsStream(resourcePath);
