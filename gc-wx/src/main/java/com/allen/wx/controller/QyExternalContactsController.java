...
import com.allen.wx.domain.QyCustomVo;
import com.allen.wx.utils.WxAESInfo;
import com.allen.wx.utils.WxAESUtil;
import com.allen.wx.utils.XStreamUtils;
...
public class QyExternalContactsController {
    ...
    public String callbackExternalUserMsgXStream(HttpServletRequest request, HttpServletResponse response,@PathVariable String corpId, @RequestBody String postData){
        ...
        try {
            XStream xstream = XStreamUtils.create(QyCustomVo.class);
            QyCustomVo msgVo = (QyCustomVo) xstream.fromXML(postData);
            msgVo.setXmlStr(postData);

            if (!StringUtils.isEmpty(msgVo.getEncrypt())) {
            ...
            if (!StringUtils.isEmpty(msgVo.getXmlStr())){
